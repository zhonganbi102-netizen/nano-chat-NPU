...
torchrun --nproc_per_node=8 -m scripts.chat_eval -- -i rl -a GSM8K
"""
...
def run_offline_test(model, tokenizer, engine, max_new_tokens=256):
    """离线简单测试（当网络不可用时）"""
    print0("🔄 运行离线数学推理测试...")
    ...
    test_problems = [
        "What is 25 times 37?",
        "John has 5 apples and buys 3 more. Then he gives 2 to Mary. How many does he have?", 
        "A rectangle has length 8 cm and width 6 cm. What is its area?",
        "If x + 5 = 12, what is x?",
        "What is 15% of 200?"
    ]
    ...
    for i, (question, expected) in enumerate(zip(test_problems, expected_answers)):
        print0(f"Problem {i+1}: {question}")
        ...
        try:
            ...
            # 解码回答
            prefix_len = len(conversation_tokens)
            response_tokens = results[0][prefix_len:]
            response_text = tokenizer.decode(response_tokens).lower()
            
            # 检查是否包含期望答案
            is_correct = any(str(ans).lower() in response_text for ans in expected)
            if is_correct:
                correct += 1
            ...
            print0(f"❌ Generation failed: {e}")
    
    accuracy = correct / total
    print0(f"Offline Test Result: {correct}/{total} ({100*accuracy:.1f}%)")
    return accuracy
...
def run_chat_eval(task_name, model, tokenizer, engine,
                   ...
                   max_problems=None):
    # Handle offline test
    if task_name == "OFFLINE":
        return run_offline_test(model, tokenizer, engine, max_new_tokens)
...
if __name__ == "__main__":
    ...
    baseline_accuracies = {
        ...
        'OFFLINE': 0.0, # offline test => 0%
    }
    
    # 如果没有指定任务，优先尝试GSM8K（RL模型专长），失败则用离线测试
    if args.task_name is None:
        task_names = ['GSM8K']  # 默认只测试GSM8K
        print0("💡 默认测试GSM8K数学推理任务（RL模型专长）")
        print0("   如需测试其他任务: -a 'ARC-Easy|MMLU|HumanEval'")
        print0("   如需离线测试: -a OFFLINE")
    else:
