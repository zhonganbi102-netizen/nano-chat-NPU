...
export MASTER_PORT=29502  # 使用新端口避免冲突
...
# 4. 创建补丁脚本
echo "4. 创建补丁脚本..."
cat > ./temp_4npu_fixed_patch.py << EOL
import sys
import torch
from nanochat.gpt import GPT

def fixed_setup_optimizers(self, unembedding_lr=0.001, embedding_lr=0.01, matrix_lr=0.01, weight_decay=0.0):
    """修复版优化器设置 - 4NPU分布式"""
    print("使用修复版NPU兼容优化器设置 (4NPU): 全部AdamW")
...
# 应用补丁
GPT.setup_optimizers = fixed_setup_optimizers
print("✅ 修复版4NPU分布式优化器补丁已应用")
EOL

# 5. 开始4NPU训练 - 使用修复的参数传递方式
echo "5. 开始修复版4NPU分布式训练..."
python -c "import sys; sys.path.insert(0, '.'); import temp_4npu_fixed_patch" && \
